...
Drawable::Drawable(OpenGLContext* context)
    : bufIdx(), bufPos(), bufNor(), bufCol(), bufInfl(), bufWeight(),
      idxBound(false), posBound(false), norBound(false), colBound(false),
      inflBound(false), weightBound(false),
      mp_context(context)
{}
...
void Drawable::generateInfl()
{
    inflBound = true;
    // Create a VBO on our GPU and store its handle in bufNor
    mp_context->glGenBuffers(1, &bufInfl);
}
...
bool Drawable::bindInfl()
{
    if(inflBound){
        mp_context->glBindBuffer(GL_ARRAY_BUFFER, bufInfl);
    }
    return inflBound;
}
